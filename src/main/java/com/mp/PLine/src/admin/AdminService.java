...
public class AdminService {
    ...
    public List<ReportResponseDto> readReports() throws BaseException {
        return reportRepository.findAllByStatus(Status.A).stream()
                .map(report -> {
                    Long feedOrCommentId = report.getFeedOrCommentId();
                    return Report.toReportResponse(report, getContext(report, feedOrCommentId));
                })
                    .collect(Collectors.toList());
    }

    private String getContext(Report report, Long feedOrCommentId) {
        String context = "";
        try {
            switch (report.getCategory()) {
                case "F":
                    context = feedRepository.findByIdAndStatus(feedOrCommentId, Status.A)
                            .orElseThrow(() -> new BaseException(BaseResponseStatus.INVALID_FEED))
                            .getContext();
                    break;
                case "C":
                    context = commentRepository.findByIdAndStatus(feedOrCommentId, Status.A)
                            .orElseThrow(() -> new BaseException(BaseResponseStatus.INVALID_COMMENT))
                            .getContext();
                    break;
                case "R":
                    context = replyRepository.findByIdAndStatus(feedOrCommentId, Status.A)
